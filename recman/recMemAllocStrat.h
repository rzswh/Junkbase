...
public:
    ...
    MemAllocStrategy(int *fe, int recordSize, int slotNum)
        : firstEmpty(fe), slotNum(slotNum), slotBytesNum((7 + slotNum) / 8),
...
public:
    RecordMemAllocateStrategy(int recordSize, int *fe)
        : MemAllocStrategy(fe, recordSize,
    ...
    int &nextEmpty(void *buf) { return ((int *)buf)[0]; }
...
public:
    VariantMemAllocateStrategy(int recordSize, int *fe)
        : MemAllocStrategy(fe, 64, (PAGE_SIZE - 48) / (1 + 64 * 8) * 8)
    {
    }
    int inline slotPos(int slot) override { return 64 + slot * recordSize; }
    int &nextEmpty(void *buf) { return ((int *)buf)[1]; }
    int &previousEmpty(void *buf) { return ((int *)buf)[2]; }
    char &bitMapByte(void *buf, int pos) override
    {
