...
class FileHandle
{
    ...
    // int inline slotPos(int slot) { return 4 + slotBitmapSizePerPage + slot *
    // recordSize; }
public:
    ...
    int insertDataAt(void *d_ptr, int length, MemAllocStrategy *strat,
                     RID &rid);
    ...
    class iterator
    {
        iterator(FileHandle *fh, RID rid, int attrLength, int attrOffset,
    ...
    iterator findRecord(int attrLength, int attrOffset, Operation compOp,
                        const void *value);
    iterator findRecord(const ComposedOperation &op);
    iterator findRecord();

private:
    // for scanning
    RID nextRecord(RID prev, int attrLength, int attrOffset, Operation compOp,
                   const void *value);
