...
int RecordManager::createFile(const char *file_name, int record_size)
{
    ...
    if (!fm->createFile(file_name)) return CREATE_FILE_FAILURE;
    int fileId;
    if (!fm->openFile(file_name, fileId)) return OPEN_FILE_FAILURE;
...
FileHandle::FileHandle(int fid, BufPageManager *bm) : fileId(fid), bpman(bm)
{
    int pg_index;
    ...
    recordMemAllocStrat =
        new RecordMemAllocateStrategy(recordSize, &firstEmptyPage);
    varMemAllocStrat = new VariantMemAllocateStrategy(VARIANT_SEGMENT_LENGTH,
                                                      &firstEmptyVarDataPage);
...
int FileHandle::insertDataAt(void *d_ptr, int length, MemAllocStrategy *strat,
                             RID &rid)
{
    ...
    for (int i = 0, off = 0; i < 2; i++) {
        while (strat->bitMapByte(buf, off) == -1 && off < slotBitmapSizePerPage)
            off++;
        ...
        //
        int index = 0;
        unsigned char bm_c = strat->bitMapByte(buf, off);
        while (bm_c & 1)
            index++, bm_c >>= 1; // Big-Endian byte order
...
int FileHandle::insertVariant(char *data, int length, RID &rid)
{
    ...
    for (int i = segNum - 1; i >= 0; i--) {
        ...
        insertDataAt(buf, varMemAllocStrat->recordSize, varMemAllocStrat,
                     lastRID);
    }
    rid = lastRID;
    return 0;
...
int FileHandle::removeData(const RID &rid, MemAllocStrategy *strat)
{
    ...
    const int slotBitmapSizePerPage = strat->slotBytesNum;
    for (int i = 0; full && i < slotBitmapSizePerPage; i++)
        full = strat->bitMapByte(buf, i) == -1;
...
// if full -> not full, mark this page as firstEmptyPage and old
// 'firstEmptyPage' next page
...
void FileHandle::updateHeader()
{
    ...
    HeaderPage header_page = HeaderPage(recordSize, firstEmptyPage,
                                        firstEmptyVarDataPage, totalPages);
                          ...
                          MemAllocStrategy *ms)
{
    ...
    if (header == 0) {
        // allocate a new page
        header = totalPages + 1;
        if (this->totalPages < header) this->totalPages = header;
...
FileHandle::iterator FileHandle::findRecord(int attrLength, int attrOffset,
                                            Operation compOp, const void *value)
{
    RID rid = nextRecord(RID(1, -1), attrLength, attrOffset, compOp, value);
    return FileHandle::iterator(this, rid, attrLength, attrOffset, compOp,
                                value);
}

FileHandle::iterator FileHandle::findRecord()
{
    return findRecord(0, 0, Operation(Operation::EVERY, TYPE_INT), this);
}

FileHandle::iterator FileHandle::findRecord(const ComposedOperation &op)
{
    if (op.num == 0) return findRecord();
    bool ok;
    FileHandle::iterator iter =
        findRecord(op.lengths[0], op.offsets[0], op.oprs[0], op.data[0]);
    do {
        ok = true;
        MRecord mrec = *iter;
        for (int i = 1; i < op.num; i++) {
            if (!op.oprs[i].check(mrec.d_ptr + op.offsets[i], op.data[i],
                                  op.lengths[i])) {
                ok = false;
                ++iter;
                break;
...
RID FileHandle::nextRecord(RID prev, int attrLength, int attrOffset,
                           Operation compOp, const void *value)
{
...
void FileHandle::debug()
{
    ...
    for (int i = 1; i <= totalPages; i++) {
        ...
        int next = recordMemAllocStrat->nextEmpty(buf),
            prev = recordMemAllocStrat->previousEmpty(buf);
...
FileHandle::iterator::iterator(FileHandle *fh, RID rid, int attrLength,
                               int attrOffset, Operation compOp,
                               const void *value)
    : fh(fh), rid(rid), attrLength(attrLength), attrOffset(attrOffset),
      comp_op(compOp), value(value)
{
}
...
bool FileHandle::iterator::end() const { return RID::end() == rid; }
